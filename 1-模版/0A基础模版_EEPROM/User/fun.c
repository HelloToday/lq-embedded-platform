...
u8 write_buff[9] = {0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38};
u8 read_buff[9];

double write_float = 1.214;
double read_float = 0.0;

void eep_init(void)
{
  
  eeprom_write_buff(0x00,write_buff,strlen((char*)write_buff));
  eeprom_read_buff(0x00,read_buff,8);
  
  //eeprom_write_buff_float(0x08,&write_float);
  //eeprom_read_buff_float(0x08,&read_float);
  eep_write_double(0x08,&write_float);
  eep_read_double(0x08,&read_float);
}
...
void lcd_part0(void)
{
	...
	sprintf((char*)lcd_text[1]," %s                 ",read_buff);
	sprintf((char*)lcd_text[2]," %.3f               ",read_float);
