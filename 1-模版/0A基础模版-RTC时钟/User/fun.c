...
void lcd_part0(void)
{
  ...
  HAL_RTC_GetDate(&hrtc,&my_sDate,RTC_FORMAT_BIN);
	...
	sprintf((char*)lcd_text[1],"Date:%d-%d-%d",my_sDate.Year,my_sDate.Month,my_sDate.Date);
	sprintf((char*)lcd_text[2],"Time:%2d:%2d:%2d",my_sTime.Hours,my_sTime.Minutes,my_sTime.Seconds);
...
void HAL_RTC_AlarmVBEventCallback(RTC_HandleTypeDef *hrtc)
{
  led3_flag++;
  if(led3_flag == 2)
    led3_flag = 0;
...
//__________________________key1_mode____________________//
	if(key_mode[0])
	{
		...
		{
      my_sTime.Minutes++;
			HAL_RTC_SetTime(&hrtc,&my_sTime,RTC_FORMAT_BIN);
		}
