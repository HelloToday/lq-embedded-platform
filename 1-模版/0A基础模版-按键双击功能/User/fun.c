...
void lcd_part0(void)
{
	...
	//sprintf((char*)lcd_text[1],"     %d               ",key_double_flag);
			...
			case 1:
			{
				if(key_state[i] == 1 &&key_last_state[i] == 1&&(HAL_GetTick() - key_press_time[i] > 1000))//³¤°´
				{
					...
					key_step[i] = 0;					
								...
								(((HAL_GetTick() - key_press_time[i] > 400)&&(HAL_GetTick() - key_press_time[i] < 1000)))  )//µ¥»÷
				{
					key_mode[i] = 1;
					key_step[i] = 0;
					
				}
				else if(key_state[i] == 0 &&key_last_state[i] == 1 && (HAL_GetTick() - key_press_time[i] < 400))//Ë«»÷
				{
					key_mode[i] = 3;
					key_step[i] = 0;
				}				
