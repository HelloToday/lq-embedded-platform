...
float r37_volt = 0, r38_volt = 0, mp_volt = 0,mp_r_volt = 0;
uint16_t mp_adc_value = 0;
uint8_t mcp_value = 0;
...
void lcd_part0(void)
{
	...
	sprintf((char*)lcd_text[4],"mp_adc:%d ",mp_adc_value);
	sprintf((char*)lcd_text[5],"mcp_val:%d",mcp_value);
	sprintf((char*)lcd_text[6],"                    ");
	sprintf((char*)lcd_text[7],"mov_r:%.2f   ",mp_r_volt);
...
void volt_adc_get(void)
{
  ...
  mp_r_volt = 3.3f*mcp_value*0.7874f/(10+mcp_value*0.7874f);
